...
# Family name per line, deduplicated.
...
install_font_family() {
  local family="$1"
  if fc-list 2>/dev/null | grep -qiF "$family"; then
    return 0
  fi
  local css_family="${family// /+}:ital,wght@0,400;0,700;1,400;1,700"
  local css urls u n=0
  css=$(curl -fsSL "https://fonts.googleapis.com/css2?family=${css_family}")
  # Some families lack bold/italic axes; retry with the bare family name.
  if [ -z "$css" ]; then
    css=$(curl -fsSL "https://fonts.googleapis.com/css2?family=${family// /+}") || return 1
  fi
...
while IFS= read -r family; do
  [ -n "$family" ] || continue
  if install_font_family "$family"; then
    fonts_ok="${fonts_ok:+$fonts_ok, }$family"
  else
